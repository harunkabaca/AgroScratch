...
export class SequentialExecutor {
  ...
  async execute(codeString) {
    ...
    const robot = {
      ...
      async highlightBlock(id) {
        ...
        self.onStepChange({ action: 'highlight', blockId: id });
        // Çok hızlı geçmemesi için minik bir bekleme
        await self.api.wait(50);
      },
      async moveTo(x, y, z) {
        await this.checkAbort();
        await this.checkPause();
  ...
  pause() {
    this._paused = true;
    this.onLog({ type: 'warn', text: '⏸ Program duraklatıldı' });
  }

  resume() {
    this._paused = false;
    this.onLog({ type: 'info', text: '▶ Program devam ediyor' });
  }
}
